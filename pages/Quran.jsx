...
const Quran = () => {
    ...
    const url =
        "https://api.quranwbw.com/v1/verses?verses=17:1,17:111&verse_translation=1&between=true";
    ...
    useEffect(() => {
        if (
            surahAyah.length >= parseInt(totalLetter) &&
            parseInt(totalLetter) > 0 &&
            parseInt(totalLetter) <= 114
        ) {
            setLoading(true);
            fetch(
                `https://api.quranwbw.com/v1/verses?verses=${JSON.stringify(
                    surahAyah[totalLetter - 1]
                )
                    .split('"')
                    .join("")
                    .replace(/\s/g, "")}&verse_translation=1&between=true`
            )
                .then((res) => res.json())
                .then((data) => {
                    setLoading(false);
            ...
            fetch(
                `https://cdn.jsdelivr.net/gh/fawazahmed0/quran-api@1/editions/ben-muhiuddinkhan/${totalLetter}.min.json`
            )
                .then((res) => res.json())
                .then((data) => {
                ...
                <div className="min-h-screen grid gap-10">
                    <div>
                        <img
                            src="/bismillah.png"
                            alt="bismillah"
                            className="w-1/6 mx-auto mt-5 mb-2"
                        />
                        <Input
                            type="text"
                            allowClear
                    ...
                    <div>
                        ...
                        totalLetter.split("").map((i, index) => {
                            ...
                            {!verses && !parseInt(totalLetter) && (
                                <p>Surah not found</p>
                            )}
                            <div className="grid gap-3">
                                {parseInt(totalLetter) <= 114 &&
                                    !loading &&
                                    Object.keys(verses).map((el, i) => (
                                        <div key={i}>
                                            <p>
                                                {verses[el].words.arabic
                                                    .split("|")
                                                    .join(" ")}
                                                <span
                                                    style={{ fontSize: "45px" }}
